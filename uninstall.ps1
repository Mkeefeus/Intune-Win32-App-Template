...
$LogFile = "C:\ProgramData\Microsoft\IntuneManagementExtension\Logs\Applications\$AppName\uninstall.log"
...
.EXAMPLE
   Add-TextToCMLog "Application of MS15-031 failed" Apply_Patch 3
...
# Start uninstall
Add-TextToCMLog -Component "Uninstall $AppName" -Severity 1 -Value "Begining uninstall of $AppName"

$App = Get-InstalledApplications -GlobalAndAllUsers | Where-Object { $_.DisplayName -like $AppName }
$ExitCode = 0
if ($App) {
    Add-TextToCMLog -Component "Get-InstalledApplications" -Severity 1 -Value "$AppName found, uninstalling"
    $App | ForEach-Object {
        $UninstallString = $_.UninstallString
        if ($UninstallString -match "msiexec.exe /I") {
            $UninstallString = $UninstallString -replace "/I", "/X"
        }
        Write-Host "Uninstalling using $UninstallString"
        $Program, $Arguments = $UninstallString -split ' ', 2
        $uninstaller = Start-Process -FilePath $Program -ArgumentList "$($Arguments) /q" -Wait -PassThru -Verb RunAs
        Add-TextToCMLog -Component "Uninstall $AppName" -Severity 1 -Value "Uninstaller exited with code $($uninstaller.ExitCode)"
        $ExitCode = $uninstaller.ExitCode
        Start-Sleep -Seconds 10
    }
}
else {
    Add-TextToCMLog -Component "Get-InstalledApplications" -Severity 1 -Value "$AppName not found"
    exit 1
}
...
$App = Get-InstalledApplications -GlobalAndAllUsers | Where-Object { $_.DisplayName -like $AppName }
if ($App) {
    Add-TextToCMLog -Component "Get-InstalledApplications" -Severity 2 -Value "$AppName is still installed"
    Add-TextToCMLog -Component "Uninstall $AppName" -Severity 3 -Value "Uninstall of $AppName failed. Application still installed."
    $ExitCode = 0 # Exit 0 here so Intune fails on the detection script post install.
}
else {
    Add-TextToCMLog -Component "Get-InstalledApplications" -Severity 1 -Value "$AppName was not detected after uninstall"
}
...
Add-TextToCMLog -Component "Uninstall $AppName" -Severity $FinalSeverity -Value "Uninstall of $AppName complete. Exit code: $ExitCode"
exit $ExitCode